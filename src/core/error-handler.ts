...
export class ErrorHandler {
  ...
  private enhanceError(error: any, context: ErrorContext): EnhancedError {
    ...
    }, this.getErrorMessage(error, context)) as EnhancedError
  ...
  private categorizeError(error: any): string {
    ...
    if (code === 429 || message.includes('rate limit') || message.includes('quota exceeded')) {
      return 'rate-limit'
    }
    
    if (code === 402 || message.includes('billing') || message.includes('payment')) {
      return 'billing'
    }
  ...
  private getUserFriendlyMessage(error: EnhancedError): string {
    const messages: Record<string, string> = {
      'authentication': 'Authentication failed. Please check your API key and ensure it has the necessary permissions.',
      'rate-limit': 'Rate limit exceeded. Please wait a moment before trying again.',
      'billing': 'There is an issue with your billing or quota. Please check your account status.',
      'invalid-request': 'The request contains invalid parameters. Please check your input and try again.',
      ...
      'timeout': 'The request timed out. Please try again with a shorter prompt or wait a moment.',
      'not-found': 'The requested resource was not found. Please verify the endpoint or model name.',
      'permission': 'You do not have permission to perform this action.',
      'unknown': 'An unexpected error occurred. Please try again or contact support if the issue persists.'
    }
    
    return messages[error.category || 'unknown']
  }
  ...
  private isRetryable(error: EnhancedError): boolean {
    const retryableCategories = ['rate-limit', 'server-error', 'network', 'timeout']
  ...
  private getRecommendations(error: EnhancedError): string[] {
    ...
    switch (error.category) {
      case 'authentication':
        ...
        recommendations.push('Check if the API key has been revoked or expired')
        recommendations.push('Ensure the API key has the necessary permissions')
        break
      
      case 'rate-limit':
        recommendations.push('Implement request queuing or throttling')
        recommendations.push('Consider upgrading your API plan for higher limits')
      ...
      case 'invalid-request':
        recommendations.push('Review the API documentation for correct parameters')
        recommendations.push('Validate input data before sending requests')
        recommendations.push('Check for any recent API changes or deprecations')
        break
