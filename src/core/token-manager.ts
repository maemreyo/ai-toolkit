...
export class TokenManager {
  ...
  async countTokens(text: string, model: string): Promise<number> {
    try {
      ...
      if (model.includes('gpt') || model.includes('davinci') || model.includes('turbo')) {
   ...
   */
  async getTokenInfo(text: string, model: string, maxTokens?: number): Promise<TokenInfo> {
    const count = await this.countTokens(text, model)
    const limit = maxTokens || this.tokenLimits[model] || this.tokenLimits.default
  ...
  async splitIntoChunks(
    ...
    for (const sentence of sentences) {
      ...
      if (currentTokens + sentenceTokens > chunkSize && currentChunk) {
        ...
        // Handle overlap
        if (overlap > 0) {
          const overlapText = await this.getOverlapText(currentChunk, model, overlap)
  ...
  async estimateConversationTokens(
    ...
    for (const message of messages) {
      ...
      totalTokens += await this.countTokens(`${message.role}: ${message.content}`, model)
